...
#sudo docker stop `sudo docker ps -a | grep container-edge-cross-compile | awk '{print $1}'`
num_container=`sudo docker ps -a | grep ${MY_CONTAINER} | awk '{print $1}'`
if [ $num_container ]; then sudo docker stop $num_container;fi
#sudo docker rm `sudo docker ps -a | grep container-edge-cross-compile | awk '{print $1}'`
if [ $num_container ]; then sudo docker rm $num_container;fi

# 4.rmi docker image
#sudo docker rmi `sudo docker images | grep ubuntu16.04_edge-cross-compile | awk '{print $3}'`
